  ...
  return (
    <>
      <style>{`
        ...
        .g-card { transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .g-card:hover { transform: translateY(-3px); box-shadow: 0 20px 60px rgba(0,0,0,0.4) !important; }
        .g-row:hover  { background: rgba(255,255,255,0.04) !important; }
        .g-btn:hover  { background: rgba(76,201,240,0.2) !important; }
      `}</style>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div className="g-card" style={{ ...glass('rgba(255,255,255,0.04)', 'rgba(255,255,255,0.08)'), padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', animation: 'fadeSlideUp 0.4s ease' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <p style={{ color: '#4cc9f0', fontSize: '15px', fontWeight: '700', margin: 0, fontFamily: 'monospace', letterSpacing: '0.05em' }}>
                {currentTime.toLocaleTimeString()}
              </p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10px', margin: '2px 0 0', fontFamily: 'monospace' }}>
                {currentTime.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
              </p>
              <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '9px', margin: '1px 0 0', letterSpacing: '0.08em' }}>
                LAST SYNC: {updated.toLocaleTimeString()}
              </p>
            </div>
          ...
          {[
            ...
            <div key={label} className="g-card" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '10px', fontWeight: '700', letterSpacing: '0.12em', margin: 0 }}>{label.toUpperCase()}</p>
                <span style={{ fontSize: '18px' }}>{icon}</span>
              </div>
              <p style={{ color, fontSize: '36px', fontWeight: '800', margin: '0 0 6px', fontFamily: 'monospace', lineHeight: 1, textShadow: `0 0 20px ${color}88` }}>{value}</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
          ...
          {/* Timeline */}
          <div className="g-card" style={{ ...glass('rgba(255,255,255,0.03)', 'rgba(255,255,255,0.07)'), padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.2s both' }}>
                  ...
                  <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', height: '100%', justifyContent: 'flex-end' }}>
                    ...
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', height: '90px' }}>
                      <div style={{
                        ...
                        boxShadow:    isToday ? '0 0 16px rgba(76,201,240,0.5)' : 'none',
                    ...
                    <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '9px' }}>{label.slice(5)}</span>
                  </div>
                )
              })}
            </div>
          ...
          {/* Severity Matrix */}
          <div className="g-card" style={{ ...glass('rgba(255,255,255,0.03)', 'rgba(255,255,255,0.07)'), padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.25s both' }}>
                ...
                return (
                  <div key={key}>
                    ...
                    <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '999px', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: `linear-gradient(90deg, ${color}, ${color}88)`, borderRadius: '999px', boxShadow: `0 0 10px ${color}66`, transition: 'width 0.8s ease' }} />
                    </div>
                  </div>
                )
              })}
        ...
        {/* ── HEATMAP ── */}
        <div className="g-card" style={{ ...glass('rgba(255,255,255,0.03)', 'rgba(255,255,255,0.07)'), padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.3s both' }}>
        ...
        {/* ── LIVE THREAT FEED ── */}
        <div className="g-card" style={{ ...glass('rgba(255,255,255,0.02)', 'rgba(255,255,255,0.07)'), overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.35s both' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {(data?.recent_threats ?? []).slice(0, 8).map((t, i) => (
                <tr key={t.threat_id} className="g-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background 0.2s', animation: `fadeSlideUp 0.3s ease ${i * 0.04}s both` }}>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <span style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '10px', fontWeight: '700', letterSpacing: '0.06em', background: `${SEV_COLOR[t.severity] || '#4cc9f0'}15`, color: SEV_COLOR[t.severity] || '#4cc9f0', border: `1px solid ${SEV_COLOR[t.severity] || '#4cc9f0'}30`, backdropFilter: 'blur(4px)' }}>
                      {t.severity}
                    </span>
                  </td>
                  <td style={{ padding: '13px 16px' }}>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <span style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '10px', fontWeight: '700', letterSpacing: '0.06em', background: `${STATUS_COLOR[t.status] || '#6b7280'}12`, color: STATUS_COLOR[t.status] || '#6b7280', border: `1px solid ${STATUS_COLOR[t.status] || '#6b7280'}28`, backdropFilter: 'blur(4px)' }}>
                      {t.status}
                    </span>
                  </td>
                </tr>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          {/* AWS Coverage — LIVE from API */}
          <div className="g-card" style={{ ...glass('rgba(255,255,255,0.03)', 'rgba(255,255,255,0.07)'), padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.4s both' }}>
            <h2 style={{ color: '#f8fafc', fontSize: '14px', fontWeight: '700', margin: '0 0 4px', letterSpacing: '0.05em' }}>AWS Service Coverage</h2>
            <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '11px', margin: '0 0 18px' }}>
              Live pipeline health · Last scan: {data?.aws_status?.last_scan ? new Date(data.aws_status.last_scan).toLocaleTimeString() : 'checking...'}
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {[
                ...
                { service: 'EventBridge', key: 'eventbridge', icon: '🔀', detail: (s: any) => s?.enabled_rules !== undefined ? `${s.enabled_rules} rules active` : '' },
                ...
                const color   = healthy ? '#06d6a0' : status === 'CHECKING' ? '#ffd166' : '#ff4d6d'
                return (
                  <div key={service} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: `${color}06`, border: `1px solid ${color}18`, borderRadius: '12px', backdropFilter: 'blur(10px)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ fontSize: '14px' }}>{icon}</span>
                      <div>
                        <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '12px', fontWeight: '500' }}>{service}</span>
                        {detail(svcData) && (
                          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '10px', margin: 0 }}>{detail(svcData)}</p>
                        )}
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: color, boxShadow: `0 0 6px ${color}`, animation: healthy ? 'pulse-glow 2s infinite' : 'none' }} />
                      <span style={{ color, fontSize: '10px', fontWeight: '700', letterSpacing: '0.08em' }}>{status}</span>
                    </div>
                  </div>
                )
              })}
          ...
          {/* Platform Risk Score */}
          <div className="g-card" style={{ ...glass('rgba(255,255,255,0.03)', 'rgba(255,255,255,0.07)'), padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.06)', animation: 'fadeSlideUp 0.5s ease 0.45s both' }}>
